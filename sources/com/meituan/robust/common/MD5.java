...
public class MD5 {
    ...
    public static void main(String[] strArr) {
        if (strArr.length == 0) {
            try {
                String hashString = getHashString(new File("/Users/hedingxu/robust-github/Robust/app/build/outputs/robust/old/resources.arsc"));
                String hashString2 = getHashString(new File("/Users/hedingxu/robust-github/Robust/app/build/outputs/robust/app-release/resources.arsc"));
                System.err.println("Resourcesarsc:");
                System.err.println("oldMd5:" + hashString);
                System.err.println("newMd5:" + hashString2);
            } catch (IOException e) {
                e.printStackTrace();
            }
            try {
                String hashString3 = getHashString(new File("/Users/hedingxu/robust-github/Robust/app/build/outputs/robust/old/AndroidManifest.xml"));
                String hashString4 = getHashString(new File("/Users/hedingxu/robust-github/Robust/app/build/outputs/robust/app-release/AndroidManifest.xml"));
                System.err.println("manifest.xml:");
                System.err.println("oldMd5:" + hashString3);
                System.err.println("newMd5:" + hashString4);
                return;
        ...
        for (String str : strArr) {
            try {
                System.out.println(getHashString(new File(str)) + StringUtil.SPACE + str);
    ...
    public static String getHashString(InputStream inputStream) throws IOException {
        MD5 md5 = new MD5();
        byte[] bArr = new byte[1024];
        while (true) {
            int read = inputStream.read(bArr);
    ...
    public static byte[] getHash(File file) throws IOException {
        ...
        byte[] hash = getHash(fileInputStream);
    ...
    public static String getHashString(File file) throws IOException {
        FileInputStream fileInputStream = new FileInputStream(file);
        String hashString = getHashString(fileInputStream);
        fileInputStream.close();
        return hashString;
    }
    ...
    public static String getHashString(String str, String str2) throws UnsupportedEncodingException {
        MD5 md5 = new MD5();
        md5.update(str, str2);
        return md5.getHashString();
    }
    ...
    private int[] decode(byte[] bArr, int i, int i2) {
        ...
        for (int i4 = 0; i4 < i; i4 += 4) {
            this.decodeBuffer[i3] = (bArr[i4 + i2] & 255) | ((bArr[(i4 + 1) + i2] & 255) << 8) | ((bArr[(i4 + 2) + i2] & 255) << 16) | ((bArr[(i4 + 3) + i2] & 255) << 24);
            i3++;
        }
