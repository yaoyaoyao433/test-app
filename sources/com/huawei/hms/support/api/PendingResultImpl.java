...
public abstract class PendingResultImpl<R extends Result, T extends IMessageEntity> extends InnerPendingResult<R> {
    ...
    public void setResult(int i, IMessageEntity iMessageEntity) {
        ...
        if (statusCode != statusCode2) {
            HMSLog.e(TAG, "rstStatus code (" + statusCode + ") is not equal commonStatus code (" + statusCode2 + CommonConstant.Symbol.BRACKET_RIGHT);
            HMSLog.e(TAG, "rstStatus msg (" + statusMessage + ") is not equal commonStatus msg (" + statusMessage2 + CommonConstant.Symbol.BRACKET_RIGHT);
        } else if (!TextUtils.isEmpty(statusMessage) || TextUtils.isEmpty(statusMessage2)) {
        } else {
            HMSLog.i(TAG, "rstStatus msg (" + statusMessage + ") is not equal commonStatus msg (" + statusMessage2 + CommonConstant.Symbol.BRACKET_RIGHT);
    ...
    protected R onError(int i) {
        ...
        if (type2 != null) {
            try {
                this.result = (R) type2.newInstance();
