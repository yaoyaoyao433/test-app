...
public final class CacheInterceptor implements Interceptor {
    ...
    public final Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (request == null && response2 == null) {
            return new Response.Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(TbsListener.ErrorCode.INFO_CODE_MINIQB_STARTMINIQBTOLOADURL_FAILED).message("Unsatisfiable Request (only-if-cached)").body(Util.EMPTY_RESPONSE).sentRequestAtMillis(-1L).receivedResponseAtMillis(System.currentTimeMillis()).build();
        }
        if (request == null) {
    ...
    private Response cacheWritingResponse(final CacheRequest cacheRequest, Response response) throws IOException {
        ...
        final d a = m.a(body);
        return response.newBuilder().body(new RealResponseBody(response.header("Content-Type"), response.body().contentLength(), m.a(new s() { // from class: okhttp3.internal.cache.CacheInterceptor.1
